function [result] = caffe_process_batch(net,img,scale,stride_h,stride_w,crop_size_h,crop_size_w,mean_value,batch_size,num_class);
 img = imresize(img,scale,'bilinear');
 [Image_patch,row_num,col_num] = ImageToGrid(img,stride_h,stride_w,crop_size_h,crop_size_w,mean_value);
 ...
 for i = 1:num_iter
    ...
    if(i~=num_iter||(i==num_iter&mod(size(Image_patch,4),batch_size)==0))
    score(:,:,:,(i-1)*batch_size+1:1:i*batch_size) = caffe_process_data(net,batch_data,crop_size_h,crop_size_w,mean_value);
    else tmp = caffe_process_data(net,batch_data,crop_size_h,crop_size_w,mean_value);
