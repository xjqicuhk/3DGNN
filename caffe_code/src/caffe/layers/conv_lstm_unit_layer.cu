...
template <typename Dtype>
__global__ void LSTMConvActsForward(const int nthreads, const int dim,
...
template <typename Dtype>
__global__ void LSTMConvUnitForward(const int nthreads, const int dim,
...
template <typename Dtype>
void LSTMConvUnitLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  //LOG(INFO) << "lstm_conv layer";
  const int count = top[1]->count();
  //bool intialize = this->layer_param_.recurrent_param().initialize();
  //LOG(INFO) << "lstm_conv layer";
 // LOG(INFO)<< initialize_;
  //LOG(INFO)<< bottom[0]->num()<<bottom[0]->channels()<<bottom[0]->height()<<bottom[0]->width();
   if(initialize_)
  {
    caffe_gpu_set(bottom[0]->count(), Dtype(0.0), bottom[0]->mutable_gpu_data());
  }
  //LOG(INFO)<< initialize_;
  ...
  LSTMConvActsForward<Dtype><<<CAFFE_GET_BLOCKS(X_count), CAFFE_CUDA_NUM_THREADS>>>(
      X_count, hidden_dim_, X, X_acts);
  CUDA_POST_KERNEL_CHECK;
  // NOLINT_NEXT_LINE(whitespace/operators)
  LSTMConvUnitForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count, hidden_dim_, C_prev, X_acts, C, H);
  CUDA_POST_KERNEL_CHECK;
}

template <typename Dtype>
__global__ void LSTMConvUnitBackward(const int nthreads, const int dim,
...
template <typename Dtype>
__global__ void LSTMConvActsBackward(const int nthreads, const int dim,
...
template <typename Dtype>
void LSTMConvUnitLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  LSTMConvUnitBackward<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
  ...
  LSTMConvActsBackward<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
...
INSTANTIATE_LAYER_GPU_FUNCS(LSTMConvUnitLayer);
