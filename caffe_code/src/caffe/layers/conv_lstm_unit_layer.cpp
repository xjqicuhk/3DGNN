...
void LSTMConvUnitLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
 ...
 // CHECK_EQ(4 * hidden_dim_, bottom[1]->shape(2));
  ...
  initialize_ = this->layer_param_.recurrent_param().initialize();
}
...
#ifdef CPU_ONLY
STUB_GPU(LSTMConvUnitLayer);
#endif

INSTANTIATE_CLASS(LSTMConvUnitLayer);
REGISTER_LAYER_CLASS(LSTMConvUnit);
