...
template <typename Dtype>
  __global__ void PixelFeatureXYForwardGPU(const int nthreads,
    const int height, const int width, const Dtype pos_scale,
    const Dtype offset_h, const Dtype offset_w, Dtype* top_data) {
...
template <typename Dtype>
  __global__ void PixelFeatureXYZForwardGPU(const int nthreads, const Dtype* bottom_depth,
    const int height, const int width, const Dtype pos_scale,
    const Dtype offset_h, const Dtype offset_w, Dtype* top_data) {
      CUDA_KERNEL_LOOP(index, nthreads) {
        const int spatial_dim = height * width;
        ...
        Dtype depth =  bottom_depth[n*1*spatial_dim +s];
        top_data[top_offset_1] = (pos_scale * y + offset_h -238.4439) * depth / 582.6910;
        top_data[top_offset_2] = (pos_scale * x + offset_w - 313.0448) * depth/ 582.6245;
        top_data[top_offset_3] = depth;
      }
  }
  template <typename Dtype>
  __global__ void PixelFeatureXYZXYForwardGPU(const int nthreads, const Dtype* bottom_depth,
    const int height, const int width, const Dtype pos_scale,
    const Dtype offset_h, const Dtype offset_w, Dtype* top_data) {
      CUDA_KERNEL_LOOP(index, nthreads) {
        const int spatial_dim = height * width;
        ...
        int top_offset_2 = ((n * out_dim + 1) * spatial_dim + s);
        int top_offset_3 = ((n * out_dim + 2) * spatial_dim + s);
        int top_offset_4 = ((n * out_dim + 3) * spatial_dim + s);
        int top_offset_5 = ((n * out_dim + 4) * spatial_dim + s);
        Dtype depth =  bottom_depth[n*1*spatial_dim +s];
        top_data[top_offset_1] = 10 * (pos_scale * y + offset_h -238.4439) * depth / 582.6910;
        top_data[top_offset_2] = 10 * (pos_scale * x + offset_w - 313.0448) * depth/ 582.6245;
        top_data[top_offset_3] = 10 * depth;
        top_data[top_offset_4] = 0.0001 * (pos_scale * y + offset_h);
        top_data[top_offset_5] = 0.0001 * (pos_scale * x + offset_w);
      }
  }
template <typename Dtype>
  __global__ void PixelFeatureXYRGBForwardGPU(const int nthreads,
    const Dtype* bottom_data,
    const int height, const int width, const int in_dim, const Dtype pos_scale,
    const Dtype color_scale, const Dtype offset_h, const Dtype offset_w,
    Dtype* top_data) {
      CUDA_KERNEL_LOOP(index, nthreads) {
        const int spatial_dim = height * width;
        const int n = index / spatial_dim;
...
template <typename Dtype>
  __global__ void PixelFeatureRGBForwardGPU(const int nthreads,
    const Dtype* bottom_data,
    const int height, const int width, const int in_dim,
    const Dtype color_scale, Dtype* top_data) {
      CUDA_KERNEL_LOOP(index, nthreads) {
        const int spatial_dim = height * width;
        const int n = index / spatial_dim;
        const int s = index % spatial_dim;
...
template <typename Dtype>
  __global__ void PixelFeatureWARPPOSForwardGPU(const int nthreads,
    ...
    const Dtype pos_scale, const Dtype angle,
    const Dtype angle_sigma, const Dtype cosAngle,
    const Dtype sinAngle, Dtype* top_data) {
      CUDA_KERNEL_LOOP(index, nthreads) {
        const int spatial_dim = height * width;
        const int n = index / spatial_dim;
        const int s = index % spatial_dim;
        ...
        top_data[top_offset_1] = sinAngle * (scaled_x - mid_x)
          + cosAngle * (scaled_y - mid_y) + mid_y;
        top_data[top_offset_2] = cosAngle * (scaled_x - mid_x)
          - sinAngle * (scaled_y - mid_y) + mid_x;
...
template <typename Dtype>
  __global__ void PixelFeatureRANDROTATEForwardGPU(const int nthreads,
    const int height, const int width,
    const Dtype pos_scale, const Dtype cosAngle,
    const Dtype sinAngle, Dtype* top_data) {
      CUDA_KERNEL_LOOP(index, nthreads) {
        const int spatial_dim = height * width;
        const int n = index / spatial_dim;
...
*/
template <typename Dtype>
void PixelFeatureLayer<Dtype>::Forward_gpu(
  ...
  case PixelFeatureParameter_Feature_POSITION: {
    if (!ran_once) {
      ...
      PixelFeatureXYForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, height_, width_, scale,
  ...
  case PixelFeatureParameter_Feature_POSITION_AND_RGB: {
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    PixelFeatureXYRGBForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, bottom_data, height_, width_, channels,
                                scale, color_scale,
                                offset_h, offset_w, top_data);
    break;
  }
  case PixelFeatureParameter_Feature_RGB_AND_POSITION: {
    ...
    const int channels = bottom[0]->channels();
    // NOLINT_NEXT_LINE(whitespace/operators)
    PixelFeatureRGBXYForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, bottom_data, height_, width_, channels,
                                scale, color_scale,
                                offset_h, offset_w, top_data);
    break;
  ...
  case PixelFeatureParameter_Feature_RGB: {
    ...
    const int channels = bottom[0]->channels();
    // NOLINT_NEXT_LINE(whitespace/operators)
    PixelFeatureRGBForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, bottom_data, height_, width_, channels,
                                color_scale, top_data);
    break;
  }
  ...
  case PixelFeatureParameter_Feature_WARPED_POSITION: {
    if (!ran_once) {
      ...
      const int nthreads = num_ * height_ * width_;
      // NOLINT_NEXT_LINE(whitespace/operators)
      PixelFeatureWARPPOSForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, height_, width_, scale, angle,
                                  angle_sigma, cosAngle, sinAngle, top_data);
    }
    break;
  }
  case PixelFeatureParameter_Feature_RANDOM_ROTATE: {
    if (!ran_once) {
      ...
      const int nthreads = num_ * height_ * width_;
      // NOLINT_NEXT_LINE(whitespace/operators)
      PixelFeatureRANDROTATEForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, height_, width_, scale,
                                  cosAngle, sinAngle, top_data);
    }
    break;
  }
  case PixelFeatureParameter_Feature_POSITIONXYZ: {
    if (!ran_once) {
      const Dtype scale = parameter.pos_scale();
      const Dtype offset_h = parameter.offset_h();
      const Dtype offset_w = parameter.offset_w();
      bottom_depth = bottom[1]->gpu_data();

      const int nthreads = num_ * height_ * width_;
      // NOLINT_NEXT_LINE(whitespace/operators)
      PixelFeatureXYZForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, bottom_depth, height_, width_, scale,
                                  offset_h, offset_w, top_data);
    }
    break;
  }
  case PixelFeatureParameter_Feature_POSITIONXYZXY: {
    if (!ran_once) {
      const Dtype scale = parameter.pos_scale();
      const Dtype offset_h = parameter.offset_h();
      const Dtype offset_w = parameter.offset_w();
      ...
      const int nthreads = num_ * height_ * width_;
      // NOLINT_NEXT_LINE(whitespace/operators)
      PixelFeatureXYZXYForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, bottom_depth, height_, width_, scale,
                                  offset_h, offset_w, top_data);
    }
    break;
