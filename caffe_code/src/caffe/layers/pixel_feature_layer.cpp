...
void PixelFeatureLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void PixelFeatureLayer<Dtype>::Reshape(
  const vector<Blob<Dtype>*>& bottom,
  const vector<Blob<Dtype>*>& top) {
    top[0]->Reshape(num_, channels_, height_, width_);
}
...
void PixelFeatureLayer<Dtype>::Forward_cpu(
  ...
  case PixelFeatureParameter_Feature_POSITIONXYZ: {
    if (!ran_once) {
      ...
      const Dtype offset_h = parameter.offset_h();
      const Dtype offset_w = parameter.offset_w();
      const Dtype cx_d = 313.0448;
      const Dtype cy_d = 238.4439;
      const Dtype fx_d = 582.6245;
      const Dtype fy_d = 582.6910;
      for (unsigned int n = 0; n < num_; ++n) {
        for (unsigned int y = 0; y < height_; ++y) {
          Dtype y_offset = scale * y + offset_h;
          for (unsigned int x = 0; x < width_; ++x) {
            top_data[top[0]->offset(n, 0, y, x)] = (y_offset - cy_d) * bottom[1]->data_at(n, 1, y, x)/fx_d;
            top_data[top[0]->offset(n, 1, y, x)] = (scale * x + offset_w - cx_d) * bottom[1]->data_at(n, 1, y, x)/fy_d;
            top_data[top[0]->offset(n, 2, y, x)] = bottom[1]->data_at(n, 1, y, x);
          }
        }
      }
    }
    ...
    if (!ran_once) {
      ...
      for (unsigned int n = 0; n < num_; ++n) {
        for (unsigned int y = 0; y < height_; ++y) {
          Dtype y_offset = scale * y + offset_h;
          for (unsigned int x = 0; x < width_; ++x) {
            top_data[top[0]->offset(n, 0, y, x)] = 10*(y_offset - cy_d) * bottom[1]->data_at(n, 1, y, x)/fx_d;
            top_data[top[0]->offset(n, 1, y, x)] = 10*(scale * x + offset_w - cx_d) * bottom[1]->data_at(n, 1, y, x)/fy_d;
            top_data[top[0]->offset(n, 2, y, x)] = 10* bottom[1]->data_at(n, 1, y, x);
            top_data[top[0]->offset(n, 3, y, x)] = 0.0001* y_offset;
            top_data[top[0]->offset(n, 4, y, x)] = 0.0001*(scale * x + offset_w);
          }
        }
      }
    }
