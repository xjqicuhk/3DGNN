...
void BNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  var_eps_ = param.var_eps();
  decay_ = param.decay();
  moving_average_ = param.moving_average();
  axis_ = param.axis();
...
void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->phase_ == TRAIN) {
    caffe_cpu_axpby(channels_, decay_, batch_statistic_.cpu_data(), Dtype(1) - decay_,
                    this->blobs_[2]->mutable_cpu_data());
  }
  if (this->phase_ == TEST && moving_average_) {
    // use moving average mean
    caffe_copy(channels_, this->blobs_[2]->cpu_data(), batch_statistic_.mutable_cpu_data());
  }
  ...
  caffe_cpu_gemv<Dtype>(CblasNoTrans,
                        nc, hw_,
                        Dtype(1. / hw_), buffer_blob_.cpu_data(),
  ...
  if (this->phase_ == TEST && moving_average_) {
    ...
    caffe_copy(channels_, this->blobs_[3]->cpu_data(), batch_statistic_.mutable_cpu_data());
  }
  // add eps
  // batch_statistic_ = batch_statistic_ + var_eps_
  caffe_add_scalar(channels_, var_eps_, batch_statistic_.mutable_cpu_data());
  // std
  // batch_statistic_ = sqrt(batch_statistic_)
  caffe_powx(channels_, batch_statistic_.cpu_data(), Dtype(0.5),
             batch_statistic_.mutable_cpu_data());
  ...
  caffe_copy(channels_, batch_statistic_.cpu_data(), x_std_.mutable_cpu_data());
...
void BNLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  // statistic across batch
  caffe_cpu_gemv<Dtype>(CblasTrans,
                        num_, this->blobs_[0]->count(),
                        Dtype(1), spatial_statistic_.cpu_data(),
  ...
  caffe_cpu_gemv<Dtype>(CblasNoTrans,
                        nc, hw_,
                        Dtype(1), const_top_diff,
                        spatial_sum_multiplier_.cpu_data(), Dtype(0),
                        spatial_statistic_.mutable_cpu_data());
  // statistic across batch
  caffe_cpu_gemv<Dtype>(CblasTrans,
                        num_, this->blobs_[1]->count(),
                        Dtype(1), spatial_statistic_.cpu_data(),
  ...
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,
                        num_, this->blobs_[0]->count(), 1, Dtype(1),
                        batch_sum_multiplier_.cpu_data(), scale_data, Dtype(0),
                        spatial_statistic_.mutable_cpu_data());
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,
                        nc, hw_, 1, Dtype(1),
  ...
  caffe_cpu_axpby(nchw, Dtype(1), buffer_blob_.cpu_data(),
                  Dtype(-1. / (num_ * hw_)), bottom_diff);

  // variance normalization
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,
                        num_, x_std_.count(), 1, Dtype(1),
